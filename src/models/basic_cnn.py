...
class BasicCNN(nn.Module):
    ...
    def __init__(self, input_height, input_width):
        ...
        outpool_h, outpool_w = conv2d_output_dim(out1_h, 2,
                                                 stride=2), conv2d_output_dim(
            out1_w, 2, stride=2)
        self.conv2 = nn.Conv2d(6, 8, 5)
        out2_h, out2_w = conv2d_output_dim(outpool_h, 5), conv2d_output_dim(
            outpool_w, 5)
        pool2_h, pool2_w = conv2d_output_dim(out2_h, 2,
                                             stride=2), conv2d_output_dim(
            out2_w, 2, stride=2)
